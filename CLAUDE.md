...
Logic is split across two files: `devconfig/src/devconfig/model.py` (Pydantic models — `Jar`, `DevConfigModel`, `DevConfigServiceModel`) and `devconfig/src/devconfig/bin/main.py` (Typer CLI, render logic). Data flow:
...
2. **DevConfig** (`devconfig.json` in the target project's cwd) — declares a config `name` and a list of `services`, each with a `name` and optional `type` (`spring` | `web`) and `path`. A `spring` service may also declare `springServiceReferences`: a map of spring property key → service name (string) or list of service names, whose URLs are written into its `application-default.yml`.
3. **Render** — alongside the per-service vars, three config-level vars are emitted: `{NAME}_WORK_NAME` (the cwd directory name), `{NAME}_DIR` (the absolute cwd / work path), and `{NAME}_WORKTREE_DIR` (its parent, i.e. the git worktree root). Each service gets a `*_PORT` env var; `spring`/`web` types also get a `*_URL`. `spring` services additionally have `application-default.yml` written into their `path` with `server.port`, plus any `springServiceReferences` (string → one URL, list → comma-joined URLs; referenced services must have a URL). Env var names are built by `_key()`: parts joined with `_`, uppercased (e.g. `NAMUHX_BACKEND_ADMIN_API_PORT`).
